"""Implementation of the ``thymira resume`` command."""
...
def resume_run(
    ...
    """Resume a run from its latest checkpoint through the Thymira API."""
    try:
    ...
    try:
        receipt = client.resume_run(run_id)
        run = settle_enqueue(client, receipt)
    except RunNotFoundError as error:
        typer.echo(render_error(str(error)), err=True)
        raise typer.Exit(code=1) from error
    except ApiError as error:
        typer.echo(render_error(str(error), "No run was resumed."), err=True)
        raise typer.Exit(code=1) from error

    typer.echo(render_run_update(run, action="resumed"))
